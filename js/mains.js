...
const visibleIds = () => [...document.querySelectorAll(".main-card:not(.hidden)")].map(c=>c.dataset.id);

function openModal(id, cardEl){
  lastCardEl = cardEl || document.querySelector(`.main-card[data-id="${id}"]`);
  modal.hidden = false;
  document.body.style.overflow = "hidden";
  selectWeapon(id);                // モーダル表示後に生成するのでチャートが正しいサイズで描画される
